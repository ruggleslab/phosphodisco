...
def _make_parser(fun=None, help_text=None):
    ...
    if fun == "generate_config":
        parser.add_argument(
            "--config_path",
            default="phdc_custom_config.yml",
            type=pathlib.Path,
        ...
        parser.add_argument(
            "--phospho", type=pathlib.Path, required=True, help="path to phospho file"
        )
        parser.add_argument(
            "--protein", type=pathlib.Path, required=True, help="path to protein file"
        )
        parser.add_argument(
            "--sample_table",
            type=pathlib.Path,
        ...
        parser.add_argument("--min_common_values", type=int, default=6, help="")
        parser.add_argument("--top_stdev_quantile", type=float, default=0.5, help="")
        parser.add_argument("--na_frac_threshold", type=float, default=0.25, help="")
        ...
        parser.add_argument(
            "--cores", type=int, default=3, help="how many cores to use. default=3"
        )
        parser.add_argument(
            "--jobs",
            type=int,
        ...
        parser.add_argument(
            "--cluster_config", type=pathlib.Path, help="snakemake cluster config file"
        )
        parser.add_argument(
            "--dry_run",
            action="store_const",
...
def run():
    ...
    snakemake_cmd = [
        ...
        f"--cluster '{args.cluster_submit_command}'"
        if args.cluster_submit_command
        else "",
    ...
    subprocess.run(" ".join([str(i) for i in snakemake_cmd]), shell=True)
...
def generate_config():
    ...
    config_template = BytesIO(
        pkgutil.get_data("phosphodisco", config_template_dict[args.template])
    )
    ...
    if args.template == 2:
        ...
        template_yml["input_phospho"] = (
            pathlib.Path(__file__).parent
            / "data"
            / "demo"
            / "BRCA_v5.2__selected_phospho.csv"
        )
        template_yml["input_protein"] = (
            pathlib.Path(__file__).parent
            / "data"
            / "demo"
        ...
        template_yml["sample_annotations_csv"] = (
            str(args.sample_table) if args.sample_table is not None else None
        )
...
def _main(args: Optional[List[str]] = None):
    ...
    if data.normed_phospho.isnull().any().any():
        ...
        data.impute_missing_values(
            **additional_kwargs_yml.get("impute_missing_values", {})
        )
        logger.info("Finished imputing missing values")
        data.normed_phospho.to_csv("%s.normed_phospho.csv" % output_prefix)
    if args.top_stdev_percent < 100:
        data.normed_phospho = data.normed_phospho.loc[
            data.normed_phospho.std(axis=1)
            < np.percentile(
                data.normed_phospho.std(axis=1), 100 - args.top_stdev_percent
            )
        ]
        data.normed_phospho.to_csv(
            "%s.normed_phospho.top%sstdev_percent.csv"
            % (output_prefix, args.top_stdev_percent)
        )
    ...
    if args.putative_regulator_list:
        ...
        data.collect_possible_regulators(
            ...
            **additional_kwargs_yml.get("collect_possible_regulators", {}),
        )
        ...
        data.regulator_coefficients.to_csv(
            "%s.putative_regulator_coefficients.csv" % output_prefix
        )
    ...
    if args.annotations:
        ...
        data.add_annotations(
            annotations=annotations, column_types=annotation_column_types
        )
        ...
        data.annotation_association.to_csv(
            "%s.annotation_association.csv" % output_prefix
        )
